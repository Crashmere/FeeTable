...
每张表最多 5,000 条记录；PNG 默认以 3 倍分辨率绘制（宽 2,832 像素），长表自动使用 2 倍（宽 1,888 像素），最多 8,000 万像素，超过限制可按标签分开导出或使用 PDF/Excel。手机通过浏览器下载或打开图片后保存到相册，系统分享取决于浏览器和安全上下文支持。
